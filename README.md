# Korean_OCR
korean ocr project

## Korean OCR Test Web Site
1. [naver](https://clova.ai/ocr)
2. [kakao](https://vision-api.kakao.com/#ocr)
3. [synapsoft](https://www.synapsoft.co.kr/ocr)

## Document Text OCR
1. [tesseract](https://github.com/tesseract-ocr/tesseract)

## Scene Text Detection + Recognition
1. [EasyOCR](https://github.com/JaidedAI/EasyOCR)
2. [PaddleOCR](https://github.com/PaddlePaddle/PaddleOCR)
3. [mmocr](https://github.com/open-mmlab/mmocr)

## Scene Text Recognition
1. [deep-text-recognition-benchmark](https://github.com/clovaai/deep-text-recognition-benchmark)
2. [Transformer-OCR](https://github.com/fengxinjie/Transformer-OCR)

## Scene Text Recognition Data Generator
1. [TextRecognitionDataGenerator](https://github.com/Belval/TextRecognitionDataGenerator)

## Scene Text Detection
1. [CRAFT-pytorch](https://github.com/clovaai/CRAFT-pytorch)
2. [EAST](https://github.com/argman/EAST)
